...
namespace CriticalCommonLib.Services
{
    public class InventoryScanner : IDisposable
    {
        ...
        {
            if (character == null) _loadedInventories.Clear();
        }
        ...
        public unsafe void ParseGlamourChest(InventorySortOrder currentSortOrder, List<BagChange> changeSet)
        {
            ...
            for (var i = 0; i < 800; i++)
            {
                ...
                if (itemId == 10450)
                {
                    var s = glamourItem.HashCode(false).ToString();
                    var o = GlamourChest[i].HashCode(false).ToString();
                    Service.Framework.RunOnTick(() =>
                    {
                        PluginLog.Log(s + " before " + o);
                    });
                }
                if (glamourItem.HashCode(false) != GlamourChest[i].HashCode(false))
                {
                    if (itemId == 10450)
                    {
                        Service.Framework.RunOnTick(() => { PluginLog.Log("i still changed though"); });
                    }

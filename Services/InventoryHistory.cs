...
public class InventoryHistory : IDisposable
{
    ...
    {
        if (!_enabled) return;
        /*Service.Log.Verbose("Original Changes: ");
        for (var index = 0; index < inventoryChanges.Count; index++)
        {
            ...
            Service.Log.Verbose("Changed: " + fromItemDebugName + " switched to " + toDebugName + " because it " +
                              change.InventoryChangeReason);
        }

        Service.Log.Verbose("Analyzed Changes: ");*/
        ...
        foreach (var change in changes)
        {
            var fromItemDebugName = change.FromItem?.DebugName ?? "Unknown";
            var toDebugName = change.ToItem?.DebugName ?? "Unknown";
            Service.Log.Verbose("Changed: " + fromItemDebugName + " switched to " + toDebugName + " because it " + change.InventoryChangeReason);
        }
        */

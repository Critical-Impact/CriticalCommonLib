...
namespace CriticalCommonLib.Crafting
{
    public class CraftMonitor : ICraftMonitor
    {
        ...
        public CraftMonitor(IGameUiManager gameUiManager, RecipeSheet recipeSheet, IObjectTable objectTable, IClientState clientState, IFramework framework, IPluginLog pluginLog, ClassJobService classJobService, ExcelSheet<GathererCrafterLvAdjustTable> adjustSheet, RecipeLevelTableSheet recipeLevelTableSheet)
        {
        ...
        private void WatchCraft()
        {
            ...
            if (this._currentWindow == WindowName.Synthesis)
            {
                ...
                if (this._currentRecipe != null)
                {
                    ...
                    if (_currentRecipe.Base.RecipeLevelTable.RowId != 0)
                    {
                        _pluginLog.Verbose($"Stellar mission recipe level is {_currentRecipe.Base.RecipeLevelTable.RowId.ToString()}");
                        ...
                        var adjustedJobLevel = Math.Min(_classJobService.GetWksSyncedLevel(classJob), _currentRecipe.Base.RecipeLevelTable.RowId);
                        _currentRecipeTable = _recipeLevelTableSheet.GetRow(_adjustSheet.GetRow(adjustedJobLevel).RecipeLevel.RowId);
